...
現在、特に日本では、さまざまな場面でロボットを利用したり、対話したりすることが一般的になっています。最近では、数多くのレストランで猫型ケータリングロボット「BellaBot（ベラボット）」が料理を運んでくれるようになり、好評を博しています。その他にも、ソフトバンクロボティクスの人型ロボット「Pepper」は教育・サービス業に、アルデバランロボティクスの人型ロボット「Nao」は三菱東京UFJ銀行の支店に、富士ソフトのコミュニケーションロボット「Palro」は介護施設に導入されるなど、ロボットの活用が進んでいます。


ソーシャルロボットとは、社会的行動や役割に応じたルールに従って、人間や他の物理的自律エージェントと対話・コミュニケーションする自律型ロボットと定義される。人間-ロボット相互作用（HRI）の分野では、子どもから大人、高齢者まで多くの研究が行われています。AlexaやSiriのような音声アシスタント技術は過去数年で成熟し、人間とコミュニケーションできるようになりましたが、ソーシャルロボットとのコミュニケーションは、物理環境全体を移動するだけでなく、愛情を示すことができるためより楽しく、また、物理的存在感と能動的関与能力によってより社会的に有意義であることが研究で示されています。


しかし、ソーシャルロボットのプログラムには制限があり、すべてのソーシャルロボットが自由にコミュニケーションを取れるわけではありません。ソーシャルロボットの多くは、特定の作業を行うために設計されており、相手からのキーワードをもとにコミュニケーションをとるようにプログラムされています。そのため、より複雑で難しい質問をされたときに返事をすることができず、人とロボットの交流に限界がありました。しかし、人工知能の分野では自然言語処理の研究が進んでおり、OpenAIのGPT-3などのソフトウェアやアプリケーションでは、自然に会話を返すことができるようになっています。


本研究では、人工知能を利用してソーシャルロボットのコミュニケーション能力を向上させるシステムの構築を目指します。 
